...
- In the current launcher behavior, ``F20`` opens the launcher, then ``F6`` opens ``ChatGPT`` with ``side_right_fill``, ``left_shift`` opens ``Codex`` with ``external_fill``, ``right_shift`` opens ``Claude`` with ``side_right_fill``, left ``Command`` opens ``System Settings`` with ``internal_fill``, ``\`` opens ``Terminal`` with ``side_right_fill``, and right ``Command`` is a reserved no-op. ``F3`` is the local Karabiner exception: it opens ``SmartShadow`` directly and then uses dock-switch placement with ``side_left_fill``.
- Karabiner single-key shortcuts for ``F3``, ``F6``, ``left_shift``, and ``right_shift`` should be generated with ``yarn karabiner:f20``. The rule keeps ``F3`` direct for SmartShadow while ``F6`` and Shift launchers send ``F20`` plus the launcher key.
