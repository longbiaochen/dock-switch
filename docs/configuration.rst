...
- Karabiner single-key shortcuts for ``F3``, ``F6``, ``left_shift``, and ``right_shift`` should be generated with ``yarn karabiner:f20``. The rule keeps ``F3`` direct for SmartShadow and ``F6`` direct for ChatGPT while Shift launchers send ``F20`` plus the launcher key.
