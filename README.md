...
- Press left `Command` to focus or create a `Codex` window on the current left side display (`Side Display` on this Mac), falling back to the external display when that side display is offline, move the mouse to that display center, and ask Codex to focus its composer with `Escape`.
