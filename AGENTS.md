...
| Refresh Karabiner single-key launcher mapping | `yarn karabiner:f20` then `yarn karabiner:f20:check` | F3 opens SmartShadow directly; F6 opens ChatGPT directly; left/right Shift send `F20` plus the launcher key | Inspect `/Users/longbiao/.config/karabiner/karabiner.json` with `jq empty` and rerun the script |
