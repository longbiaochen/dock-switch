...
var KEY_MAP = { "ArrowLeft": "2", "ArrowUp": "0", "ArrowRight": "1" };
var ITEMS = [];
...
$(function() {
    $(document).on("keydown", function(e) {
        ...
        if (KEY_MAP[e.key] != undefined) {
            ...
            // new Notification(item.name, { body: e.key });
            child_process.execSync(util.format(APP_TPL, item.name));
            // check whether on Macbook Pro Retina Display
            console.log(electron.remote.screen.getPrimaryDisplay().internal);
            if (electron.remote.screen.getPrimaryDisplay().id == 69733248) {
                child_process.execSync(util.format(SCREEN_TPL, 0));
            } else {
